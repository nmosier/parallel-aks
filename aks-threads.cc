...
int main(int argc, char *argv[]) {
  ...
  const char *usage = "usage: %s [-tvjh] [n]\n";
  const char *optstring = "tvhj:";
  int optchar;
  while ((optchar = getopt(argc, argv, optstring)) >= 0) {
    ...
    case 'v':
      ...
      break;
    case 'j':
      num_threads = std::stoi(optarg, nullptr, 0);
      break;
    case 'h':
  ...
  timer main_timer("Total Execution Time");
  ...
  if (verbose) std::clog << "stage 4" << std::endl;
  {
    ...
    mt_range_for<mpz_class>(ring::zero<mpz_class>, l + ring::unity<mpz_class>, num_threads, [&](const mpz_class& a) {
